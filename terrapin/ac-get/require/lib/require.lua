...
local module_finders = {
	...
	["ac-get API Finder"] = function(module_name)
		local file_tokens = str_split(module_name, "%.")
		local searched_paths = {}

		-- Build the list of api paths:
		-- This assumes that the ac-get default dirs are respected
		if fs.exists('/lib') and fs.isDir('/lib') then
			local packages = fs.list('/lib')

			for k = 1, #packages do
				local api_folder = fs.combine('/lib', packages[k])
				...
				if fs.isDir(api_folder) then
					...
					if fs.exists(module_path) then
						...
						table.insert(searched_paths, module_path)
					end
